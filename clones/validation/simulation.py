...
class BenchmarkVisualization:
    ...
    def _scatter(self, ax, c, s=8, cmap=plt.cm.viridis, norm=None, title=None):
    ...
    def plot_measurements(self, ax=None):
        """ Scatter plot of measurement data. """
        if ax is None:
            fig, ax = plt.subplots(figsize=(2, 2))
        self._scatter(ax, c=self.fluorescence, norm=self.fnorm, title=None)

    def show(self):
        """ Plot visual comparison of cell classifiers. """
        fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(6, 6))
        self._scatter(axes[0, 0], c=self.true_genotypes, title='Ground Truth')
        self._scatter(axes[0, 1], c=self.fluorescence, norm=self.fnorm, title='Fluorescence')
        self._scatter(axes[1, 0], c=self.simple_genotypes, title='Cell-based classifier')
        self._scatter(axes[1, 1], c=self.community_genotypes, title='Community-based classifier')

    def show_measurements(self):
        """ Plot visual comparison of genotypes and measurements. """
        fig, axes = plt.subplots(ncols=2, figsize=(6, 3))
        self._scatter(axes[0], c=self.true_genotypes, title='Ground Truth')
        self._scatter(axes[1], c=self.fluorescence, norm=self.fnorm, title='Fluorescence')

    def show_comparison(self, **kwargs):
        """ Plot visual comparison of cell classifiers. """
        fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(6, 6))
        self._scatter(axes[0, 0], c=self.simple_genotypes, title='Cell-based classifier')
        self._scatter(axes[1, 0], c=self.community_genotypes, title='Community-based classifier')
